...
public class ChatEvent implements Listener {
    ...
    public void onPlayerChat(PlayerChatEvent ev){
        if(plugin.hFriend != null){
            if(plugin.hFriend.chatMode.containsKey(ev.getPlayer().getName())) return;
        }
        if(plugin.hGuild != null){
            if(plugin.hGuild.guildChat.containsKey(ev.getPlayer().getName())) return;
        }
        ...
        this.plugin.todayDB.addCount("dc_count_chat", 1);
            ...
            if(message.indexOf(s) > -1){
                ...
                if(!isSent) {
                    ...
                    if (!this.plugin.isJDALoaded) {
                        this.plugin.getLogger().warning("욕설감지 : " + ev.getPlayer().getName());
                    } else {
                        this.jdaManager.sendEmbedMessage(
                                this.jdaManager.getBadWordReportChannel().getId(), embedBuilder.build());
                    }
                    isSent = true;
                }
